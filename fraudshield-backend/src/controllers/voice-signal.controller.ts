...
export class VoiceSignalController {
    ...
    static async reportCallSignal(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const { event, duration, incomingNumber } = req.body;
            ...
            if (event === 'CALL_START' || event === 'CALL_ACTIVE') {
                ...
                logger.info(`VoiceSignal: Active call signal received for user ${userId}`);
                ...
                logger.info(`VoiceSignal: Call ended signal received for user ${userId}. Duration: ${duration}s`);
            }
